...
namespace StormDotNet.Implementations
{
    ...
    internal class StormFuncFromValues<TSource, TResult> : StormBase<TResult>
    {
        private readonly IStorm<TSource> _source;
        private readonly Func<TSource, TResult> _func;

        public StormFuncFromValues(IStorm<TSource> source,
        ...
        private void SourceOnVisit(IStormToken token, EStormVisitType visitType)
        {
            switch (visitType)
            {
                ...
                case EStormVisitType.LeaveChanged:
                    void OnError(StormError error) => LeaveWithError(token, error);
